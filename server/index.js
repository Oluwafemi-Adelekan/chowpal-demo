...
// Temporary in-memory session history
// Structure: { sessionId: [ { text: "Hi", sender: "user" }, ... ] }
const sessions = {};
...
if (!apiKey || !endpoint || !deployment) {
    console.error("FATAL: AZURE_OPENAI credentials missing in .env");
}
...
app.post('/api/chat', async (req, res) => {
    try {
        ...
        for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
            try {
                ...
                if (isRateLimit && attempt < MAX_RETRIES - 1) {
                    ...
                    console.log(`[WARN] Rate limited (attempt ${attempt + 1}/${MAX_RETRIES}). Waiting ${waitTime / 1000}s before retry...`);
        ...
        if (isRateLimit) {
            res.json({ text: "Whoa, I'm getting a lot of questions right now! 🔥 Give me a few seconds and try again. In the meantime, check out these popular items!", cards: menuItems.slice(0, 3) });
        } else {
            res.json({ text: "I'm having a little trouble reaching my brain servers. \n\nCheck out the menu items below in the meantime!", cards: [] });
        }
    }
});
...
// Endpoint to create a new session (just clear it for this demo)
...
app.listen(PORT, () => {
    console.log(`[INFO] Chowpal Backend running on port ${PORT}`);
});
